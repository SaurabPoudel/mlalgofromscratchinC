...
void forward(Layer *layer, float *input, float *output){
	...
	for(int i = 0; i < layer->input_size; i++){
		float in_i = input[i];
		float *weight_row = &layer->weights[i * layer->output_size];
		for(int j = 0; j < layer->output_size; j++){
			output[j] += in_i * weight_row[j];
		}
	}
	for (int i = 0; i < layer->output_size; i++)
...
void backward(Layer *layer, float *input, float *output_grad, float *input_grad, float lr){
	...
	if(input_grad){
		for(int i = 0; i < layer->input_size; i++){
			...
			float *weight_row = &layer->weights[i *layer->output_size];
			for(int j = 0; j < layer->output_size; j++){
				input_grad[i] += output_grad[j] * weight_row[j];
			}
		} 
	}
	...
	for(int i = 0; i < layer->input_size; i++){
		float in_i = input[i];
		float *weight_row = &layer->weights[i * layer->output_size];
		float *momentum_row = &layer->weight_momentum[i * layer->output_size];
		for(int j = 0;  j < layer->output_size; j++){
			float grad = output_grad[j] * in_i;
			momentum_row[j] = MOMENTUM * momentum_row[j] + lr * grad;
			weight_row[j] -= momentum_row[j];
			if(input_grad)
				input_grad[i] += output_grad[j] * weight_row[j];
		} 
	}
